...
const Inventory: React.FC<InventoryProps> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalBook: Book = {
      ...(formData as Book),
      id: editingBook?.id || 'book-' + Math.random().toString(36).substr(2, 9),
    ...
    if (editingBook) onUpdateBook(finalBook);
    else onAddBook(finalBook);
                  ...
                  {genreEditName?.old === cat ? (
                    ...
                    <>
                      ...
                      <div className="flex gap-4 opacity-0 group-hover:opacity-100 transition">
                        <button onClick={() => setGenreEditName({old: cat, new: cat})} className="text-stone-500 hover:text-stone-100"><i className="fas fa-pen text-[11px]"></i></button>
                        <button onClick={() => onUpdateCategories(categories.filter(c => c !== cat))} className="text-rose-900 hover:text-rose-500"><i className="fas fa-trash text-[11px]"></i></button>
                      </div>
                    </>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-fadeIn overflow-y-auto">
          ...
          <div className={`relative max-w-4xl w-full border shadow-2xl overflow-hidden animate-slideUp my-auto ${cardBg}`}>
            <div className="p-8 border-b flex justify-between items-center bg-stone-950/50">
               ...
               <button onClick={() => setIsModalOpen(false)} className="opacity-50 hover:opacity-100"><i className="fas fa-times text-lg"></i></button>
            </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                      ...
                      <input type="number" required value={formData.price} onChange={e => setFormData({...formData, price: Number(e.target.value)})} className={inputClass} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[8px] font-black uppercase tracking-widest opacity-30">Кількість на складі</label>
                      <input type="number" required value={formData.inventory} onChange={e => setFormData({...formData, inventory: Number(e.target.value)})} className={inputClass} />
                    </div>
                  </div>
                </div>
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
                       ...
                       <input type="number" value={formData.year} onChange={e => setFormData({...formData, year: Number(e.target.value)})} className={inputClass} />
                    </div>
                    <div className="space-y-1">
                       <label className="text-[8px] font-black uppercase tracking-widest opacity-30">Сторінок</label>
                       <input type="number" value={formData.pages} onChange={e => setFormData({...formData, pages: Number(e.target.value)})} className={inputClass} />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-1">
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center border-b pb-2 border-stone-800/20">
                  ...
                  <label className="cursor-pointer px-6 py-2 border text-[9px] font-black uppercase tracking-widest hover:bg-stone-100 hover:text-stone-950 transition">
                    ...
                    <input type="file" multiple accept="image/*" onChange={handleFileChange} className="hidden" disabled={isLoadingImages} />
                  </label>
                </div>
