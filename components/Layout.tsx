...
const Layout: React.FC<LayoutProps> = ({ 
  children, setActiveView, user, onLogout, isDarkMode, 
    ...
    <div className={`min-h-screen flex flex-col selection:bg-amber-200/20 selection:text-stone-100/90 transition-all duration-700`}>
      ...
      <nav className={`${navClass} backdrop-blur-md sticky top-0 z-40 border-b transition-colors duration-700`}>
        <div className="max-w-[1300px] mx-auto px-2 md:px-6 overflow-x-auto no-scrollbar">
          <div className="flex items-center justify-start md:justify-center gap-1 md:gap-2 min-w-max">
          <button onClick={() => handleViewChange('storefront')} className={`px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-[0.18em] md:tracking-[0.4em] whitespace-nowrap transition border-b-2 ${activeView === 'storefront' || activeView === 'book_details' ? (isDarkMode ? 'border-stone-100 text-stone-100' : 'border-stone-900 text-stone-900') : 'border-transparent opacity-50 hover:opacity-100'}`}>Магазин</button>
          {user && <button onClick={() => handleViewChange('profile')} className={`px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-[0.18em] md:tracking-[0.4em] whitespace-nowrap transition border-b-2 ${activeView === 'profile' ? (isDarkMode ? 'border-stone-100 text-stone-100' : 'border-stone-900 text-stone-900') : 'border-transparent opacity-50 hover:opacity-100'}`}>Кабінет</button>}
          {user?.role === 'admin' && (
            <>
              <div className="w-px h-4 mx-2 bg-stone-500/20"></div>
              <button onClick={() => handleViewChange('admin_orders')} className={`px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-[0.18em] md:tracking-[0.4em] whitespace-nowrap transition border-b-2 ${activeView === 'admin_orders' ? (isDarkMode ? 'border-stone-100 text-stone-100' : 'border-stone-900 text-stone-900') : 'border-transparent opacity-50 hover:opacity-100'}`}>Замовлення</button>
              <button onClick={() => handleViewChange('crm')} className={`px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-[0.18em] md:tracking-[0.4em] whitespace-nowrap transition border-b-2 ${activeView === 'crm' ? (isDarkMode ? 'border-stone-100 text-stone-100' : 'border-stone-900 text-stone-900') : 'border-transparent opacity-50 hover:opacity-100'}`}>CRM</button>
              <button onClick={() => handleViewChange('inventory')} className={`px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-black uppercase tracking-[0.18em] md:tracking-[0.4em] whitespace-nowrap transition border-b-2 ${activeView === 'inventory' ? (isDarkMode ? 'border-stone-100 text-stone-100' : 'border-stone-900 text-stone-900') : 'border-transparent opacity-50 hover:opacity-100'}`}>Склад</button>
            </>
          )}
          </div>
        </div>
