...
const CRM: React.FC<CRMProps> = ({ customers, orders, isDarkMode }) => {
  ...
  const stats = [
    ...
    { 
      ...
      value: `${orders.filter(o => !['closed', 'cancelled', 'received'].includes(o.status)).length}`, 
      trend: `В обробці`, 
      icon: 'fa-chart-line' 
    }
  ];
          ...
          <div className={`relative max-w-2xl w-full border shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] overflow-hidden animate-slideUp ${cardBg}`}>
            <div className={`p-10 border-b flex justify-between items-start ${headerBg} ${isDarkMode ? 'border-stone-800' : 'border-stone-100'}`}>
              ...
              <button
                ...
                aria-label="Закрити картку користувача"
                title="Закрити"
                className={`text-xl transition ${isDarkMode ? 'text-stone-700 hover:text-stone-100' : 'text-stone-300 hover:text-stone-900'}`}
              >
