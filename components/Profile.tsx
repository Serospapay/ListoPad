...
const Profile: React.FC<ProfileProps> = ({ user, books, orders, isDarkMode, wishlistBooks, onToggleWishlist, onViewBook, onLogout }) => {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'ordered':
        return { label: 'Оформлено', colorClass: isDarkMode ? 'text-rose-500 border-rose-900/40' : 'text-rose-600 border-rose-200' };
      case 'shipping':
        return { label: 'Доставляється', colorClass: isDarkMode ? 'text-orange-500 border-orange-900/40' : 'text-orange-600 border-orange-200' };
      case 'at_branch':
        return { label: 'У відділенні', colorClass: isDarkMode ? 'text-sky-500 border-sky-900/40' : 'text-sky-600 border-sky-200' };
      case 'received':
        return { label: 'Отримано', colorClass: isDarkMode ? 'text-emerald-500 border-emerald-900/40' : 'text-emerald-600 border-emerald-200' };
      default:
        return { label: 'Оформлено', colorClass: isDarkMode ? 'text-rose-500 border-rose-900/40' : 'text-rose-600 border-rose-200' };
    }
  };
