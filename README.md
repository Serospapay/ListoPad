...
- БД за замовчуванням: **MongoDB** через офіційний пакет **`django-mongodb-backend`** (узгоджений з **Django 5.2**). Опційно: `PostgreSQL`, або `DB_ENGINE=sqlite` лише для ізольованих тестів/CI без запущеного MongoDB.
...
python manage.py mongo_migrate --noinput
python manage.py runserver 127.0.0.1:8000
```

Демо-каталог (`seed_demo_books`) викликається автоматично з `run-local.bat` після міграцій; окремо вона потрібна лише при ручному запуску backend.
...
- `DB_ENGINE=mongodb` або `DB_ENGINE=sqlite` або `DB_ENGINE=postgres`
- для mongodb: `MONGODB_URI`, `MONGODB_NAME`
...
- frontend lint + тести + build
- backend migrate + тести проти **MongoDB** у сервіс-контейнері
- Playwright smoke
...
- Використовувати MongoDB Atlas або PostgreSQL для production
